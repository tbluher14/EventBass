  ...
  useEffect(() => {
    ...
    if (isLoaded) {
      ...
      if (start_date.length < 1)
        errors.push('Start Date for your event is required')
      if (start_time > end_time && start_date === end_date)
        errors.push('Start Time must be before event End Time on the same day')
      if (start_date.length > 10)
        errors.push('Please enter a valid Start Date (MM-DD-YYYY)')
      if (end_date.length > 10)
        errors.push('Please enter a valid End Date (MM-DD-YYYY)')
      if (start_date > end_date)
        errors.push("Start Date must be before the event's End Date")
      if (start_time.length < 1)
        errors.push('Start Time for your event is required')
      if (end_date.length < 1)
        errors.push('End Date for your event is required')
      if (end_time.length < 1)
        errors.push('End Time for your event is required')

      if (start_time < currentTime && start_date == currentDate())
        errors.push('Please enter a Start Time in the future.')
      if (name.length < 2 || name.length > 255)
        errors.push('Event Name must be between 2 and 255 characters')
      if (venue_name.length < 2 || venue_name.length > 50)
        errors.push('Venue Name must be between 2 and 50 Characters')
      if (description.length < 2 || description.length > 500)
        errors.push(
          'Description for your event must be between 2 and 500 Characters'
        )
      if (address.length < 2 || address.length > 50)
        errors.push(
          'Address for your event must be between 2 and 50 Characters'
        )
      if (city.length < 2 || city.length > 50)
        errors.push('City for your Event must be between 2 and 50 Characters')
      if (state.length < 2 || state.length > 50)
        errors.push('State for your event must be between 2 and 50 Characters')
      if (zip_code.length !== 5 || !zip_code.match(zipRegX))
        errors.push('Zip Code must be 5 digits in length')
      if (image_url.length < 2 || !image_url.split('?')[0].match(imageRegX))
        errors.push(
          'Image Url for your event must be of type: jpg, jpeg, png, svg'
        )
      if (
        website.length < 2 ||
        (/^https:\/\//.test(website) === false &&
          /^http:\/\//.test(website) === false)
      ) {
          ...
          <div className='form-container'>
            ...
            {submitted &&
              errors.map((error, ind) => (
                ...
                // >
                <div className='edit_event_error-message' key={ind}>
                  {error}
                </div>
                // </div>
              ))}
            ...
            <div className='form-input-container'>
              ...
              <div className='input-container'>
                <label htmlFor='Event City' className='form-label'>
                  City
                </label>
                <input
                  className='form-field'
              ...
              <div className='input-container'>
                <label htmlFor='Event State' className='form-label'>
                  State
                </label>
                <input
                  className='form-field'
              ...
              <div className='input-container'>
                <label htmlFor='Event Zip' className='form-label'>
                  Zip Code
                </label>
                <input
                  className='form-field'
              ...
              <div className='input-container'>
                <label htmlFor='Event Website' className='form-label'>
                  Event Website
                </label>
                <input
                  className='form-field'
              ...
              <div className='input-container'>
                <label
                  htmlFor='Open Time & Close Time'
                  className='form-field-labels'
                >
                  Refunds Offered?
                </label>
                <select
                  className='select-form-field-time'
                  value={refunds}
